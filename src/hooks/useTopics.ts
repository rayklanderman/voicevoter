...
export function useTopics() {
  ...
  const generateNewTopics = async () => {
    try {
      ...
      for (const topic of newTopics) {
        try {
          ...
          const existingCategory = categories.find(
            (cat) => cat.name.toLowerCase() === topic.category.toLowerCase()
          );
          ...
          if (insertError) {
            ...
            console.log(
              `✅ Stored topic: ${insertedTopic.title.substring(0, 50)}...`
            );
          }
        } catch (topicError) {
      ...
      console.log(
        `✅ Successfully generated and stored ${insertedTopics.length} new topics`
      );
    } catch (err) {
