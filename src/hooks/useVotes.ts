...
export function useVotes(questionId: string | undefined) {
  ...
  useEffect(() => {
    ...
    async function fetchVotes() {
      try {
        ...
        const yesCount = votesData?.filter(vote => vote.choice === 'yes').length || 0;
        const noCount = votesData?.filter(vote => vote.choice === 'no').length || 0;
  ...
  const castVote = async (choice: 'yes' | 'no') => {
    ...
    try {
      ...
      console.log('Vote cast successfully:', choice, user ? 'authenticated' : 'anonymous');
      return { error: null };
