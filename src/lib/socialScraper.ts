...
import {
  scrapeTrends24,
  getGlobalTrends,
  scrapeXTrendsAlternative,
} from "./trends24Scraper";
...
function calculateNewsVolume(article: any): number {
  ...
  const sourceMultiplier =
    Object.entries(sourceMultipliers).find(([key]) =>
      sourceName.includes(key)
    )?.[1] || sourceMultipliers.default;
...
// GitHub API configuration (no auth needed for public repos)
const GITHUB_API_CONFIG: SocialMediaAPI = {
  name: "GitHub",
  endpoint:
  ...
  parseResponse: (data) => {
    ...
    return data.items
      .filter((repo: any) => repo.stargazers_count > 100)
      .map((repo: any) => ({
        text: `${repo.name} - ${
          repo.description?.substring(0, 80) || "No description"
        }`,
...
// NewsAPI rate limiting and optimization with cross-tab synchronization
class NewsAPIManager {
      ...
      this.storageEventListener = (e: StorageEvent) => {
        if (e.key === this.STORAGE_KEY && e.newValue) {
          try {
            ...
            this.lastResetDate = data.date || new Date().toDateString();
          } catch (error) {
  ...
  canMakeRequest(): boolean {
    // Refresh data from storage before checking
    this.loadUsageData();
    return this.requestCount < this.MAX_DAILY_REQUESTS;
  }

  getRemainingRequests(): number {
    this.loadUsageData(); // Ensure we have latest data
  ...
  incrementUsage() {
    this.loadUsageData(); // Get latest count before incrementing
    this.requestCount++;
    this.saveUsageData();
    console.log(
      `📰 NewsAPI usage: ${this.requestCount}/${this.MAX_DAILY_REQUESTS} requests today`
    );
  }
...
export function getConfiguredAPIs(): {
  ...
  return [
    ...
    },
    {
      name: "TikTok",
      configured: !!(
        import.meta.env.VITE_TIKTOK_ACCESS_TOKEN &&
    ...
    {
      name: "GitHub",
      configured: true, // GitHub doesn't require API key for public repos
    },
    {
      name: "NewsAPI",
      configured: !!(
...
const ENHANCED_MOCK_DATA: Record<string, string[]> = {
  ...
  github: [
    "DeepSeek-V3 - Revolutionary AI model with breakthrough reasoning",
    "Anthropic Claude 4 - Advanced AI assistant with improved safety",
    "OpenAI GPT-5 - Next generation language model",
    "Google Gemini Ultra - Multimodal AI system",
    "Microsoft Copilot Studio - AI development platform",
    "Meta Llama 3.5 - Open source large language model",
    "Hugging Face Transformers 5.0 - ML model library",
    "LangChain 2.0 - AI application development framework",
    "Stable Diffusion XL - Advanced image generation",
    "AutoGPT Pro - Autonomous AI agent framework",
  ],
  news: [
  ...
  try {
    ...
    if (config.name === "NewsAPI") {
      ...
      if (!newsManager.shouldMakeRequest(priority)) {
        ...
        console.log(
          `⚠️ NewsAPI request skipped. Usage: ${stats.used}/${stats.total}, Priority: ${priority}`
        );
        throw new Error(
          `NewsAPI quota management: ${stats.remaining} requests remaining`
        );
      }
    }
...
export async function scrapeTrendingTopics(
  ...
  try {
    ...
    switch (source) {
      case "x_twitter":
        ...
        try {
          ...
          console.warn(
            "All X scraping methods failed, using enhanced mock data"
          );
          trends = [];
        }
        break;
      ...
      case "news":
        const newsConfigured = configuredAPIs.find(
          (api) => api.name === "NewsAPI"
        )?.configured;
        if (newsConfigured) {
          try {
      ...
      case "github":
        try {
          trends = await scrapeFromAPI(GITHUB_API_CONFIG, limit, priority);
        } catch (error) {
    ...
    if (trends.length > 0) {
      console.log(
        `✅ Successfully scraped ${trends.length} real trends from ${source}`
      );
...
export function getScrapingStatus(): Record<
  string,
  { configured: boolean; lastScraped?: string; usage?: any }
> {
  ...
  apis.forEach((api) => {
    ...
    status[sourceKey] = {
      ...
      lastScraped:
        localStorage.getItem(`last_scraped_${sourceKey}`) || undefined,
      usage: api.usage,
    };
  });
...
export async function smartScrapingStrategy(
  sources: string[],
  ...
  if (sources.includes("news")) {
    ...
    console.log(
      `📊 NewsAPI Daily Usage: ${usage.used}/${usage.total} (${usage.remaining} remaining)`
    );
  }
